...
TCT = sum(x_shared[d,t,k,day].varValue * truck_collection_time_per_ton * truck_capacity for d in districts for t in range(total_trucks) for k in timeslots for day in days)
# Compute TGC (Total Garbage Collected)
TGC = sum(g_shared[d,day].varValue for d in districts for day in days)
